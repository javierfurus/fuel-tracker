...
@Component({
  ...
  template: `
  <mat-toolbar color="primary" class="max-width mat-elevation-z8"><mat-toolbar class="content-container" color="primary">Dashboard</mat-toolbar></mat-toolbar>
  <mat-card class="container" style="display:flex; justify-content: space-around" >
    <div *ngIf="car" #containerRef style="width:35%;">
      <p style="text-align: center; font-size:'1.5em'; font-weight:'bold'">Tank Level</p>
          <ngx-charts-gauge [view]="[containerRef.offsetWidth, 350]" [scheme]="colorScheme" [results]="single" [legend]=false [max]="car.tankSize"	[units]="units">
    ...
    <div *ngIf='tripLeft && tripLeft.roadType !== "No trip"; else noTrip' style="padding-bottom: 0;"  >
      ...
      <p *ngIf='tripLeft.tripLeft > 0; else noTripLeft'>Trip left: ~{{tripLeft.tripLeft}} Km if you continue on {{tripLeft.roadType}}</p>
      <ng-template #noTripLeft><p>You ran out of fuel! Refill now!</p></ng-template> 
    </div>
    <ng-template #noTrip>
      <p>You have no logged trips</p>
    </ng-template>
    <p *ngIf='car'>You are driving a {{car.name}}, your current fuel level is {{car.currentFuelLevel}} L/{{car.tankSize}}</p>
    <div *ngIf='car && car.fuelPercent < 40 && !noTrip' style="display:flex; align-items: center"><p style="margin-bottom: 0;">Your car's tank is less than 40%. You should consider refilling it.</p><mat-icon style="color:orange">warning</mat-icon></div>  
    </div>
  </div>
  </mat-card>
  <div class="container" >
